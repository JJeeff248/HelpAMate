<?php
	...
	if(isset($_POST['submit']) && $_POST['submit'] != "Delete") {
		...
		if (strlen($fname) > 30 || strlen($fname) == 0 && $_SESSION['loggedin'] = 1) {
			...
			$fname = '';
		} else if (strlen($lname) > 30 || strlen($lname) == 0) {
			$check = 1;
	...
	if ($_POST['submit'] == "Sign up!") {
		$add_pledge = $dbc->prepare("INSERT INTO `Fundraiser`(`FName`, `LName`, `DoB`, `Email`, `Password`, `Charity`, `Blurb`, `Goal`) VALUES (?,?,?,?,?,?,?,?)");
		$add_pledge->bind_param("sssssssi", $fname, $lname, $userDob, $email, $bcrypt_password, $charity, $blurb, $goal);
		$add_pledge->execute();
		$add_pledge->close();
		...
		$update_fundraiser = $dbc->prepare("UPDATE `Fundraiser` SET `FName` = ?,`LName` = ?, `DoB` = ?,`Email` = ?,`Charity` = ?,
